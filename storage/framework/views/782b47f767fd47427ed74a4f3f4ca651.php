...
<?php $__env->startSection('title', 'Signatures'); ?>
<?php $__env->startSection('page-title', 'Signatures'); ?>
...
<div id="flash-success" class="mb-5 px-4 py-3 bg-green-50 border border-green-200 text-green-700 rounded-xl text-sm font-medium flex items-center gap-2">
    <i class="fa-solid fa-circle-check text-green-500"></i> <?php echo e(session('success')); ?>
...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-6">
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm font-semibold text-gray-700">
        <button onclick="scrollToSection('section-inbox')"
            ...
            En attente
            <span class="ml-auto bg-amber-500 text-white text-xs font-bold px-2 py-0.5 rounded-full"><?php echo e($pendingActionCount); ?></span>
        </button>
...
<section id="section-inbox" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
    <div class="flex items-center justify-between mb-4">
        ...
        <span class="text-xs text-gray-400 bg-gray-100 px-3 py-1 rounded-full"><?php echo e(count($workflowInbox)); ?> ligne(s)</span>
    </div>
    <?php if(count($workflowInbox) === 0): ?>
    ...
    <div class="overflow-x-auto rounded-xl border border-gray-200">
        <table class="w-full min-w-[760px] text-left text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                <?php $__currentLoopData = $workflowInbox; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <tr class="<?php echo e($row['isMyTurn'] ? 'bg-white' : 'bg-gray-50'); ?> hover:bg-blue-50/30 transition-colors">
                    <td class="px-4 py-3 font-medium text-gray-800"><?php echo e($row['workflowName']); ?></td>
                    <td class="px-4 py-3 text-gray-600 text-xs"><?php echo e($row['creatorLabel']); ?></td>
                    <td class="px-4 py-3 text-gray-600 text-xs"><?php echo e($row['documentTitle']); ?></td>
                    <td class="px-4 py-3">
                        <div class="flex items-center gap-2">
                            <div class="w-20 h-2 rounded-full bg-gray-200 overflow-hidden">
                                <div class="h-2 rounded-full bg-[#2453d6]" style="width: <?php echo e($row['progress']); ?>%"></div>
                            </div>
                            <span class="text-xs text-gray-600 font-semibold"><?php echo e($row['progress']); ?>%</span>
                        </div>
                    </td>
                    <td class="px-4 py-3">
                        <?php
                        ...
                        ?>
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold <?php echo e($sc); ?>"
                              data-exec-status-cell="<?php echo e($row['actionableIds'][0] ?? ''); ?>"
                              data-status="<?php echo e($row['status']); ?>">
                            <?php echo e($row['statusLabel']); ?>

                        </span>
                    </td>
                    <td class="px-4 py-3 text-gray-500 text-xs"><?php echo e($row['nextActorLabel']); ?></td>
                    <td class="px-4 py-3 text-right">
                        <?php if($row['status'] === 'completed' && !empty($row['signedFilePath']) && !empty($row['firstExecutionId'])): ?>
                            
                            <a href="<?php echo e(route('signatures.signed-document', $row['firstExecutionId'])); ?>"
                               target="_blank"
                        ...
                        <?php elseif($row['status'] !== 'completed' && count($row['actionableIds']) > 0): ?>
                            <?php $firstExecId = $row['actionableIds'][0]; ?>
                            <button
                                type="button"
                                onclick="wfInboxAction(this, '<?php echo e($firstExecId); ?>', '<?php echo e($row['actionType']); ?>')"
                                data-action="<?php echo e($row['actionType']); ?>"
                                data-exec="<?php echo e($firstExecId); ?>"
                                data-execution-ids='<?php echo json_encode($row['actionableIds'], 15, 512) ?>'
                                data-document-url="<?php echo e(!empty($row['firstExecutionId']) ? route('signatures.workflow-document', ['executionId' => $row['firstExecutionId']]) : ''); ?>"
                                data-workflow-name="<?php echo e($row['workflowName']); ?>"
                                data-document-title="<?php echo e($row['documentTitle']); ?>"
                                class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl font-medium text-sm transition-all duration-200 <?php echo e($row['actionType'] === 'signature' ? 'bg-blue-50 border border-blue-200 text-blue-600 hover:bg-blue-100 hover:border-blue-300 hover:shadow-md active:bg-blue-200' : 'bg-emerald-50 border border-emerald-200 text-emerald-600 hover:bg-emerald-100 hover:border-emerald-300 hover:shadow-md active:bg-emerald-200'); ?>"
                                title="<?php echo e($row['actionType'] === 'signature' ? 'Signer sur la plateforme' : 'Lire et valider le document'); ?>">
                                <i class="fa-solid <?php echo e($row['actionType'] === 'signature' ? 'fa-pen-to-square' : 'fa-circle-check'); ?> text-base wf-btn-icon"></i>
                                <span><?php echo e($row['actionType'] === 'signature' ? 'Signer' : 'Valider'); ?></span>
                            </button>
                            
                            <form id="wf-local-<?php echo e($firstExecId); ?>" method="POST" action="<?php echo e(route('signatures.workflow-action')); ?>" class="hidden">
                                <?php echo csrf_field(); ?>
                                <?php $__currentLoopData = $row['actionableIds']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $execId): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <input type="hidden" name="execution_ids[]" value="<?php echo e($execId); ?>">
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                <input type="hidden" name="action_type" value="<?php echo e($row['actionType']); ?>">
                            </form>
                        <?php else: ?>
...
<form id="workflow-validation-form" method="POST" action="<?php echo e(route('signatures.workflow-action')); ?>" class="hidden">
    <?php echo csrf_field(); ?>
...
<div class="hidden">
    <section id="section-sign">
        ...
        <div class="mb-5">
            ...
            <select id="doc-selector" onchange="syncDocSelect(this.value)"
                ...
                <?php $__currentLoopData = $documents; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $doc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($doc->id); ?>"><?php echo e($doc->title); ?><?php echo e($doc->status === 'signed' ? ' ✓' : ''); ?></option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
        </div>
        <div class="border border-gray-100 rounded-xl p-4 mb-4 bg-gray-50">
            ...
            <form method="POST" action="<?php echo e(route('signatures.store')); ?>">
                <?php echo csrf_field(); ?>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="<?php echo e(route('signatures.upload')); ?>"
        ...
        <div id="section-request" class="border border-gray-100 rounded-xl p-4 bg-gray-50">
            ...
            <form method="POST" action="<?php echo e(route('signatures.request')); ?>">
                <?php echo csrf_field(); ?>
    ...
    <section class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <h2 class="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
            ...
            <span class="ml-auto text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full font-normal"><?php echo e($mySignatures->total()); ?></span>
        </h2>
        <?php if($mySignatures->isEmpty()): ?>
        ...
        <div class="space-y-2 max-h-96 overflow-y-auto pr-1">
            <?php $__currentLoopData = $mySignatures; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sig): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="border border-gray-100 rounded-xl p-3 bg-gray-50 hover:bg-white transition">
                <p class="text-sm font-medium text-gray-800 truncate">
                    ...
                    <?php echo e($sig->document->title ?? 'Document supprime'); ?>

                </p>
                <div class="flex items-center justify-between mt-1.5">
                    <span class="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-semibold
                        <?php echo e($sig->status === 'valid' ? 'bg-green-100 text-green-700' : ($sig->status === 'revoked' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600')); ?>">
                        <i class="fa-solid fa-<?php echo e($sig->status === 'valid' ? 'check-circle' : 'ban'); ?> text-[10px]"></i>
                        <?php echo e(match($sig->status) { 'valid' => 'Valide', 'revoked' => 'Revoquee', 'expired' => 'Expiree', default => ucfirst($sig->status) }); ?>

                    </span>
                    <span class="text-xs text-gray-400"><?php echo e($sig->signed_at?->format('d/m/Y') ?? '—'); ?></span>
                </div>
            </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
        <?php if($mySignatures->hasPages()): ?>
            <div class="mt-3 text-center"><?php echo e($mySignatures->links()); ?></div>
        <?php endif; ?>
        <?php endif; ?>
    </section>
</div>
...
<section id="section-workflows" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
    <div class="flex items-center justify-between mb-5">
        ...
        <span class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-50 border border-amber-200 text-amber-700 text-xs font-semibold">
            ...
            <?php echo e($pendingActionCount); ?> en attente validation/signature
        </span>
    </div>
    <?php if($myWorkflows->isEmpty()): ?>
        <div class="text-center py-10 text-gray-400">
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody>
                <?php $__currentLoopData = $myWorkflows; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $wf): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php
                ...
                <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    <td class="px-4 py-4">
                        <p class="font-medium text-gray-900"><?php echo e($wf->name); ?></p>
                        <?php if($latestRejection): ?>
                            <div class="mt-2 flex flex-wrap items-center gap-2">
                                <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-semibold bg-red-50 text-red-700 border border-red-200">
                                    ...
                                    Dernier refus: <?php echo e($latestRejection['actor_name'] ?? 'Utilisateur'); ?> · <?php echo e(!empty($latestRejection['rejected_at']) ? \Carbon\Carbon::parse($latestRejection['rejected_at'])->format('d/m/Y H:i') : 'date inconnue'); ?>

                                </span>
                                <button
                                    type="button"
                                    onclick='openWorkflowRejectionHistoryModal(<?php echo json_encode($wf->name, 15, 512) ?>, <?php echo json_encode($rejectionHistory->values()->all(), 15, 512) ?>)'
                    ...
                    <td class="px-4 py-4">
                        <div class="flex items-center gap-2">
                            <div class="w-8 h-8 rounded-full bg-[#2453d6] flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
                                <?php echo e(strtoupper(substr($wf->creator?->name ?? $wf->creator?->email ?? '?', 0, 1))); ?>

                            </div>
                            <div class="min-w-0">
                                <p class="text-sm font-medium text-gray-900 truncate"><?php echo e($wf->creator?->name ?? '—'); ?></p>
                                <p class="text-xs text-gray-400 truncate"><?php echo e($wf->creator?->email ?? ''); ?></p>
                            </div>
                        </div>
                    </td>
                    <td class="px-4 py-4 text-xs text-gray-500"><?php echo e($wf->updated_at?->format('d/m/y H:i') ?? '—'); ?></td>
                    <td class="px-4 py-4">
                        <span class="inline-block px-3 py-1 rounded-full text-xs font-bold uppercase <?php echo e($statusColor); ?>"><?php echo e($statusLabel); ?></span>
                    </td>
                    <td class="px-4 py-4">
                        <div class="w-28 h-7 rounded border border-gray-300 bg-white overflow-hidden relative">
                            <div class="absolute inset-y-0 left-0 <?php echo e($progressColor); ?> transition-all" style="width: <?php echo e($progress); ?>%"></div>
                            <span class="absolute inset-0 flex items-center justify-center text-xs font-bold text-gray-700"><?php echo e($progress); ?> %</span>
                        </div>
                    </td>
...
<section class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-6">
    <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
        ...
        <span class="text-xs text-gray-400 bg-gray-100 px-3 py-1 rounded-full"><?php echo e($mySignatures->total()); ?> signature(s)</span>
    </div>
    <div class="overflow-x-auto">
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-50">
                <?php $__empty_1 = true; $__currentLoopData = $mySignatures; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sig): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-5 py-4">
                        <p class="text-sm font-medium text-gray-800"><?php echo e($sig->document->title ?? 'Document supprime'); ?></p>
                    </td>
                    <td class="px-5 py-4">
                        <?php
                        ...
                        <?php if($zoneReq): ?>
                            <div class="text-xs text-blue-700 bg-blue-50 border border-blue-200 rounded-lg px-2.5 py-1.5 inline-block">
                                ...
                                Page <?php echo e($zoneReq->zone_page); ?> · X <?php echo e(round($zoneReq->zone_x, 1)); ?>% · Y <?php echo e(round($zoneReq->zone_y, 1)); ?>%
                            </div>
                        <?php else: ?>
                    ...
                    <td class="px-5 py-4">
                        <span class="text-xs px-2.5 py-1 rounded-full font-semibold
                            <?php echo e($sig->status === 'valid' ? 'bg-green-100 text-green-700' : ($sig->status === 'revoked' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600')); ?>">
                            <i class="fa-solid fa-<?php echo e($sig->status === 'valid' ? 'check-circle' : 'ban'); ?> mr-1 text-xs"></i>
                            <?php echo e(match($sig->status) { 'valid' => 'Valide', 'revoked' => 'Revoquee', 'expired' => 'Expiree', default => ucfirst($sig->status) }); ?>

                        </span>
                    </td>
                    <td class="px-5 py-4 text-xs text-gray-500"><?php echo e($sig->signed_at?->format('d/m/Y H:i') ?? '—'); ?></td>
                    <td class="px-5 py-4 text-xs text-gray-500"><?php echo e($sig->reason ?? '—'); ?></td>
                </tr>
    ...
    <?php if($mySignatures->hasPages()): ?>
        <div class="px-6 py-4 border-t border-gray-100"><?php echo e($mySignatures->links()); ?></div>
    <?php endif; ?>
</section>
...
const workflowCreateUrl = <?php echo json_encode(route('workflows.index', ['create' => 'workflow', 'embedded' => '1'])) ?>;
...
const __wfInviteUrl = "<?php echo e(route('signatures.get-invite-url')); ?>";
...
const __wfStatusPollUrl = "<?php echo e(route('signatures.platform-status', ['executionId' => '__EXEC_ID__'])); ?>".replace('__EXEC_ID__', '__PLACEHOLDER__');
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\wamp64\www\e-administration_laravel.worktrees\gestion-courrier-notification-email\resources\views/signatures/index.blade.php ENDPATH**/ ?>