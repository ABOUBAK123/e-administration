@extends('layouts.app')
@section('title', 'Templates partagés')
@section('page-title', 'Templates partagés')
@section('page-subtitle', 'Modèles de documents — remplissez les variables et générez votre fichier')

@section('content')
@php use Illuminate\Support\Facades\Storage; @endphp

{{-- Données JSON sécurisées pour le JS, passées via script tag --}}
...
<div class="flex items-center gap-4 mb-6">
    <form method="GET" action="{{ route('shared-templates.index') }}" class="flex-1 max-w-md flex gap-2">
        <div class="relative flex-1">
            <i class="fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm"></i>
            <input type="text" name="q" value="{{ $search }}" placeholder="Rechercher un template…"
        ...
        @if($search)
            <a href="{{ route('shared-templates.index') }}" class="px-4 py-2.5 border border-gray-300 text-gray-600 rounded-xl text-sm hover:bg-gray-100 transition"><i class="fas fa-times"></i></a>
        @endif
    </form>
</div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition p-5 flex flex-col gap-3">
            <div class="flex items-start gap-3">
                <div class="h-12 w-12 rounded-xl {{ $extColor }} flex items-center justify-center flex-shrink-0">
                    <i class="fas {{ $extIcon }} text-xl"></i>
                </div>
                <div class="flex-1 min-w-0">
                    <h3 class="font-bold text-gray-800 text-sm leading-tight">{{ $tpl->name }}</h3>
                    <p class="text-xs text-gray-400 mt-0.5">{{ strtoupper($ft) }}</p>
                    @if($tpl->administration)
                        <p class="text-xs text-gray-500 mt-0.5 truncate">
                            <i class="fas fa-building mr-1 text-[10px]"></i>{{ $tpl->administration->name }}
                        </p>
                    @endif
                </div>
            </div>
            ...
            <div class="flex flex-wrap gap-1">
                @foreach($allVars->take(3) as $v)
                    <span class="inline-block bg-blue-50 text-blue-700 text-[10px] font-medium px-2 py-0.5 rounded-full border border-blue-100">{{ $v->label ?: $v->key }}</span>
                @endforeach
                @if($allVars->count() > 3)
                    <span class="inline-block bg-gray-100 text-gray-500 text-[10px] px-2 py-0.5 rounded-full">+{{ $allVars->count() - 3 }}</span>
                @endif
            </div>
            @elseif($tpl->content)
            ...
            <div class="flex gap-2 mt-auto">
                ...
                @if($tpl->storage_path && Storage::disk('public')->exists($tpl->storage_path))
                <a href="{{ asset('storage/' . $tpl->storage_path) }}" target="_blank"
...
{{-- ═══════════════════════════════════════
     MODAL FORMULAIRE DE GÉNÉRATION
═══════════════════════════════════════ --}}
...
<script>
(function () {
    var BASE_URL  = '{{ url("shared-templates") }}';
    var DOCS_URL  = '{{ route("documents.index") }}';
