...
export class CreateRequestedActsTable1712200000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (dbType === 'postgres') {
      ...
      await queryRunner.query(
        `CREATE INDEX IF NOT EXISTS "IDX_requested_acts_scope" ON "requested_acts" ("administrationScopeType", "administrationScopeId")`
      );
      await queryRunner.query(
        `CREATE INDEX IF NOT EXISTS "IDX_requested_acts_directionCode" ON "requested_acts" ("directionCode")`
      );
      await queryRunner.query(
        `CREATE INDEX IF NOT EXISTS "IDX_requested_acts_createdAt" ON "requested_acts" ("createdAt")`
      );
      return;
    }
    ...
    await queryRunner.query(
      ...
      CREATE TABLE IF NOT EXISTS ` +
        ...
        `),
        KEY ` +
        '`IDX_requested_acts_scope`' +
        ` (` +
        '`administrationScopeType`' +
        `, ` +
        '`administrationScopeId`' +
        `),
        KEY ` +
        '`IDX_requested_acts_directionCode`' +
        ` (` +
        '`directionCode`' +
        `),
        KEY ` +
        '`IDX_requested_acts_createdAt`' +
        ` (` +
        '`createdAt`' +
        `)
